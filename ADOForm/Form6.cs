...
namespace ADOForm
{
    public partial class Form6 : Form
    {
        ...
        private int INSERTRow()//사용자 함수 정의
        {
            ...
            string strqry = "INSERT INTO phone VALUES ('" + inid + "', '" + inName + "', '" + inPhone + "', '" + inMail + "')"; // 작은 따옴표 추가
            //"INSERT INTO phone VALUES (id, pname, phone, email)"을 수정
            OracleCommand OraCmd = new OracleCommand(strqry, odpConn);

            return OraCmd.ExecuteNonQuery(); //추가되는 행수 반환
            // ExecuteNonQuery -> DB에 INSERT  정상적으로 처리 -> 0보다 큼 , 오류 -> 0보다 작음 (리턴값)
        }
